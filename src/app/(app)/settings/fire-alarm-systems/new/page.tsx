...
export default async function NewFireAlarmSystemPage() {
  ...
  async function createSystem(formData: FormData) {
    'use server'
    const supabase = await createClient()
    await supabase.from('fire_alarm_systems').insert({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-sm font-medium text-slate-900">Add System</span>
      </div>

      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Add Fire Alarm System</h1>
        <p className="mt-1 text-sm text-slate-500">Register a fire alarm panel to use when logging alarm tests.</p>
      </div>
