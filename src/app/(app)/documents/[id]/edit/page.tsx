  ...
  useEffect(() => {
    async function load() {
      const [docRes, catRes, siteRes, userRes] = await Promise.all([
        supabase
          .from('documents')
          .select('id, title, version, status, review_due_date, site_id, owner_id, category_id')
          .eq('id', params.id)
          .single(),
        supabase
          .from('lookup_values')
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
          <a href="/documents" className="hover:text-orange-600 transition-colors">Document Library</a>
          <span>/</span>
          <a href={`/documents/${params.id}`} className="hover:text-orange-600 transition-colors">Document</a>
          <span>/</span>
          <span>Edit</span>
        </div>
